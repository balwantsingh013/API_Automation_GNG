Feature: Verify SearchAccounts TurnOff Api
  ...
  Background: Generate Authentication Token
    ...
    Scenario Outline:  Verify Response when an Valid Banner Account Number parameters with transactionType As TOFF is input For "<requestID>"
    ...
    Scenario Outline: Verify Response  when a Last Name And Zipcode And transactionType As TOFF is input With "<requestID>"
    ...
    Scenario Outline: Verify Response  when a Valid Combination Of First Name, Last Name And Zipcode with transactionType As TOFF is input For "<requestID>"
    ...
    Scenario Outline: Verify Response For Verify that when a Valid CustomerBusinessName Parameter with transactionType As TOFF is input For Commercial Active And "<requestID>"
    ...
    Scenario: Verify Response For Verify that when a Valid CustomerBusinessName Parameter with transactionType As TOFF is input For Commercial Active And  Active/Pending Rewards Account TC_83
